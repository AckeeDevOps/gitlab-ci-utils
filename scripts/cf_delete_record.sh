...
DNS_RECORD_ID=$(curl -X GET "https://api.cloudflare.com/client/v4/zones/0da3f10ded5b96d4145d94c3415c9777/dns_records" \
     -H "X-Auth-Key: 84d09b4b57f4e26825e408bb1e9b7e6416dec" \
     -H "X-Auth-Email: admin@ackee.cz" \
...
if [[ -z "${DNS_RECORD_ID}" ]]; then
    ...
    curl -X DELETE "https://api.cloudflare.com/client/v4/zones/${DOMAIN_ZONE_ID}/dns_records/${DNS_RECORD_ID}" \
        ...
        jq ".result | map(select(.id == \"${DOMAIN_ZONE_ID}\"))"
fi
